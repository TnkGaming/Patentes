...
function openWindow(){
	...
	output='data';
	newWindow = window.open('http://www.soccervista.com/soccer_games.php?date=2017-'+date, "mywin", '');
}
function getNextDay(){
...
function getData(table){
	var league='';
	var fullTimePrediction = '';
	var underoverPrediction = '';
	var score= '';
	for(var i=0;i<table.children.length;i++){
		...
		if(row.className == 'headupe'){
			...
			fullTimePrediction = getFTPrediction(row);
			underoverPrediction = getOUPrediction(row);getOUPrediction
			score = getScore(row);
			dataToPrint+=league+','+fullTimePrediction+','+underoverPrediction+','+score+'\n';
		}
	}
}
...
function getFTPrediction(row){
	try{
		var prediction = row.getElementsByTagName('strong')[1].innerHTML;
		return prediction == undefined || prediction == '' ? '' : prediction ;
	}catch(e){
		return '';
	}
}
function getOUPrediction(row){
	try{
		var prediction = row.children[10].innerHTML;
	return prediction == undefined || prediction == '' ? '' : prediction ;
	}catch(e){
		return '';
	}
